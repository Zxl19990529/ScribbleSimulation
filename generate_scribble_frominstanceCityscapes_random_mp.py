...
def imgs2scribble(args,imginfos,dataDir,dataType,saveDir,tqdm_id):
    ...
    for id,imginfo in tqdm(enumerate(imginfos),total=len(imginfos),ncols=70,position=tqdm_id):
        ...
        img_path = os.path.join(dataDir,dataType,basename+'.jpg')
        ...
        for one_ann  in current_anns:
            if one_ann['iscrowd']:
                ...
                try:
                    ...
                    f=open(scribble_output_dir+'_Error_record.txt','a')
        ...
        background_mask = cv2.rectangle(background_mask,(1,1),(width-1,height-1),color=(0,0,0),thickness=2) # rectangle the whole image.
        background_mask = cv2.rectangle(background_mask,(0,int(height/2)-100),(width,int(height/2)+100),color=(0,0,0),thickness=-1) # rectangle the center area to split the image into up and bottom regions.
        ...
        largest_area = 0
        skel_list = []
        for region_id  in np.unique(regions): # skip the non-interest area.
            if region_id == 0:
                continue
            curr_skel = (regions == region_id)
            skel_list.append(curr_skel)
            # if np.sum(curr_skel) >=largest_area:
            #     largest_area = np.sum(curr_skel)
            #     clean_skel = curr_skel
        for clean_skel in skel_list:
        # if clean_skel is not None:
            skeleton_canvas[clean_skel] = 255
            max_depth_path = get_dfs_points(clean_skel.astype(np.int32))
            # scribble_points = get_bezier_points(np.array(max_depth_path))
            scribble_points = scribble_points[1:-1,:]
            # try:
            #     # scribble_points = get_bezier_points(np.array(max_depth_path))
            #     # scribble_points = np.array(scribble_points)
        ...
        cv2.imwrite(os.path.join(vis_skeleton_outputdir,basename+'.png'),skeleton_canvas)
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataDir',default='./Cityscapes_COCOstyle',type=str)
    parser.add_argument('--dataType',default='ValImages',type=str)
    parser.add_argument('--save_dir',default='./cityscape_scribble_r1',type=str)
    ...
    for one_info in tqdm(imginfos,ncols=70,total=len(imginfos),desc='Checking...'): # to skip the completed ones
        ...
        basename = filename.split('/')[-1].split('.')[0]
        filepath = os.path.join(saveDir,'scribble_gt',basename+'.png')
    ...
    for taskid,task_imginfo in enumerate(task_split):
        ...
        # p.apply(imgs2scribble,(args,task_split[taskid],dataDir,dataType,saveDir,taskid))
    p.close()
    p.join()